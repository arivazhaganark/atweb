...
<?php
   $filemanager = App\Model\backend\FileManagerModel::where('permission', 'im')->where('parent_id', 0)->first();
...
<script type="text/javascript">
    $(function () {   
        $('.back').attr('value', '');
        $('#back1, #back2').hide();
        ajaxfm('<?php echo $file_name; ?>', '<?php echo $file_id; ?>', "");   
        ...
        $('.back').click(function(){ 
            ...
            $.ajax({
                 type: "POST",
                 url: app_url + "/brochure/filemanager/parentinfo",
    ...
    function fnTs(dir, file_id) {
       ...
       $.ajax({
           type: "POST",
           url: app_url + "/gallery/filemanager/ajaxfm",
